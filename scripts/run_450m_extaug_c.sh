...
DATA=artifacts/datasets/mixed/pater_mixed_extaug_c.jsonl
OUT=artifacts/checkpoints/qwen_450m_extaugc_s${SEED}
EVAL_VAL=artifacts/reports/qwen_450m_extaugc_eval_s${SEED}.json
EVAL_TRAIN=artifacts/reports/qwen_450m_extaugc_traineval_s${SEED}.json
...
python3 -u scripts/train_lm_aux.py --config "${CFG}" --tokenizer "${TOK}" --dataset "${DATA}" \
  ...
  --tag "450m_extaugc_s${SEED}" --log-interval 50
echo "TRAIN_EXIT=$?"
