...
DATA=artifacts/datasets/mixed/pater_mixed_realext.jsonl
OUT=artifacts/checkpoints/qwen_450m_realext_s${SEED}
EVAL_VAL=artifacts/reports/qwen_450m_realext_eval_s${SEED}.json
EVAL_TEST=artifacts/reports/qwen_450m_realext_test_s${SEED}.json
...
python3 -u scripts/train_lm_aux.py --config "${CFG}" --tokenizer "${TOK}" --dataset "${DATA}" \
  ...
  --tag "450m_realext_s${SEED}" --log-interval 50
echo "TRAIN_EXIT=$?"
