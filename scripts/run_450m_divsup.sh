#!/usr/bin/env bash
# 450M diversified+support-coverage gate: identical data/config/tokenizer/recipe as
# run_450m_stability_divsup.sh, only a longer schedule (default 1800 steps,
...
# Usage: run_450m_divsup_recover.sh <seed> <gpu> [steps] [peak_lr] [warmup]
set -euo pipefail
...
DATA=artifacts/datasets/mixed/pater_mixed_25_75_divsup.jsonl
OUT=artifacts/checkpoints/qwen_450m_divsup_s${SEED}
EVAL_VAL=artifacts/reports/qwen_450m_divsup_eval_s${SEED}.json
EVAL_TRAIN=artifacts/reports/qwen_450m_divsup_traineval_s${SEED}.json
